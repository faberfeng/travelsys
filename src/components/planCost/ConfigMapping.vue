                    ...
                    <div >
                        <zk-table 
                            ...
                            <template slot="action" slot-scope="scope">
                                ...
                                <button class="listBtn actionBtn" v-if="scope.row.level == 4" title="构件列表" @click="entityList(scope)"></button>
                                <button class="detailBtn actionBtn" v-if="scope.row.level == 4" title="编辑映射" @click="editMapping(scope)"></button>
                            </template> 
                        </zk-table>
                    </div>
                </div>
...
import mappingproject from './MappingProject.vue';
export default {
    name:'ConfigMap',
    data(){
        return{
    ...
    methods:{
        ...
        refreshGouJian(){
            ...
            axios({
                method:'post',
                url:this.BDMSUrl+'project2/report/refreshEntityList',
                headers:{
                    token:this.token
                },
                params:{
                    buildId:this.singleValue,
                    districtId:this.sperateValue,
                    projId:this.projId,
                    expandNodeNo:'[]'
                }
            }).then(response=>{
       ...
       editMapping(scope){
            ...
            this.projectNumber = scope.row.classifyCode.split('-')[1];
            this.fourthSelectTitle = this.projectNumber.substr(6,3)+'-'+scope.row.engineerName;
            axios({
                method:'get',
                url:this.BDMSUrl+'project2/Config/loadLevelXGenieClass',
                headers:{
            ...
            axios({
                ...
                params:{
                    obscureCode:scope.row.classifyCode.split('-')[1].substr(0,2)+'__00',
                    codeLength:6,
                    tableNo:'t32',
                    projId:this.projId,
                    type:2
            ...
            axios({
                ...
                params:{
                    obscureCode:scope.row.classifyCode.split('-')[1].substr(0,4)+'__',
                    codeLength:6,
                    tableNo:'t32',
                    projId:this.projId,
                    type:2
