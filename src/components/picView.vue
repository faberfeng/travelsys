  ...
  >
    ...
    >
      ...
      <canvas
        ...
        id="drawCanvas"
        style="position: absolute; top: 0px; left: 0px"
        @dblclick="Select_item"
        @mousedown="oncanvasmousedown"
        @mousemove="oncanvasmousemove"
        @mouseup="oncanvasmouseup"
      ></canvas>
      <canvas
        ref="drawCanvasSelect"
        id="drawCanvasSelect"
      ...
      >
        <ul>
          <li
            ...
            v-for="(item, index) in pointGroupDataList"
            :key="index"
            @click="itemOnCleck(item)"
          >
...
export default {
  ...
  mounted() {
    ...
    this.$refs.picView.style.width =
      this.$refs.picView.parentNode.offsetWidth + "px";
    this.$refs.picView.style.height =
      this.$refs.picView.parentNode.offsetHeight + "px";
    this.old_para = "";
  ...
  methods: {
      ...
      switch (type) {
        ...
        case "PNG":
...

          {
            ...
            this.isRender = true;
            console.log(source, "source99");
            this.image.src = source;
            this.image.onload = (e) => {
              this.ResolutionScale =
                (1.0 * this.sub_div.offsetWidth) / this.image.width;

              this.oldImageSize = {
                width: this.sub_div.offsetWidth,
                height:
                  this.sub_div.offsetWidth *
    ...
    load() {
      ...
      var timer = setInterval(() => {
        this.sub_div.style.height =
          this.$refs.pdfDocument.$refs.canvasParent.offsetHeight + "px";
        this.oldImageSize = {
        ...
        this.PDFsize = {
          width: this.oldImageSize.width,
          height: this.oldImageSize.height,
        };
    ...
    setNumber_inputPostion(position_) {
      ...
      this.$refs.number_input.style.left =
        this.sub_div.offsetLeft +
        position.x * this.ResolutionScale * this.scale -
        50 +
        "px";
      this.$refs.number_input.style.top =
        this.sub_div.offsetTop +
    ...
    oncanvasmousedown(e) {
      ...
      if (e.button == 0 || e.button == 2) {
        ...
        if (this.drawCount > 0 && this.drawing) {
          //  绘制标记
          this.drawCount--;
                ...
                case "cloud":
                  if (this.drawList[i].position.length > 2) {
                    let dist = new THREE.Vector2(
                      X - this.drawList[i].position[0].x,
                      Y - this.drawList[i].position[0].y
                    ).length();
                    if (dist < 10) {
                      this.drawList[i].position.push({
                        x: this.drawList[i].position[0].x,
                        y: this.drawList[i].position[0].y,
                      });
                      this.drawCount = 0;
          ...
          if (find == false) {
            ...
            this.drawList.push(
              this.create(this.drawtype, this.drawItemId, { x: X, y: Y })
            );
          ...
          if (this.drawCount == 0) {
            ...
            if (
              this.drawList[this.drawList.length - 1].type == "Select_img_Mark"
            ) {
              this.setDrawCancel();
            } else {
          ...
          if (this.needChangeBroken) {
            var selectColorID = this.drawcontextSelect.getImageData(
              e.layerX,
              e.layerY,
              1,
              1
                ...
                for (let i = 0; i < this.drawList.length; i++) {
                  ...
                  if (this.drawList[i].SID == SID) {
                    ...
                    if (this.Selected_typeNum_List.length >= this.Max_type) {
                      ...
                      for (
                        let j = 0;
                        j < this.Selected_typeNum_List.length;
                        j++
                      ) {
                        if (
                          this.Selected_typeNum_List[j] ==
                          this.drawList[i].typeNum
                        ) {
                          typeNum_enable = true;
                        }
                      }
                      ...
                      if (!find_type) {
                        this.Selected_typeNum_List.push(
                          this.drawList[i].typeNum
                        );
                      }
                    }
              ...
              for (let i = 0; i < this.drawList.length; i++) {
                ...
                if (this.drawList[i].SID == SID) {
                  ...
                  if (this.Selected_typeNum_List.length >= this.Max_type) {
                    ...
                    if (!find_type) {
                      this.Selected_typeNum_List.push(this.drawList[i].typeNum);
                    }
                  }
            ...
            if (this.editStatus == "move" && SID != 0) {
              ...
              for (let i = 0; i < this.SelectedList.length; i++) {
                this.SelectedList[i].TempPostion.x =
                  this.SelectedList[i].position[0].x - center.x;
                this.SelectedList[i].TempPostion.y =
                  this.SelectedList[i].position[0].y - center.y;
              }
    ...
    oncanvasmousemove(e) {
      ...
      if (this.drawList.length > 0) {
        if (
          this.drawList[this.drawList.length - 1].type == "text" &&
          this.drawList[this.drawList.length - 1].position.length != 2
        ) {
      ...
      if (this.startMove) {
        for (let i = 0; i < this.SelectedList.length; i++) {
          ...
          for (let j = 0; j < this.SelectedList[i].position.length; j++) {
            var position = {};
            position.x =
              this.SelectedList[i].position[j].x -
              this.SelectedList[i].position[0].x;
            position.y =
              this.SelectedList[i].position[j].y -
              this.SelectedList[i].position[0].y;
            relative_position.push(position);
          }

          for (let j = 0; j < this.SelectedList[i].position.length; j++) {
            this.SelectedList[i].position[j].x =
              X + this.SelectedList[i].TempPostion.x + relative_position[j].x;
            this.SelectedList[i].position[j].y =
              Y + this.SelectedList[i].TempPostion.y + relative_position[j].y;
          }
        }
      }
    ...
    position_calculate() {
      ...
      var Vector = new THREE.Vector2(
        Vector_before_dir.x *
          ((Vector_before_length * this.scale) / this.scale_before),
        Vector_before_dir.y *
          ((Vector_before_length * this.scale) / this.scale_before)
      );
    ...
    Refresh() {
      ...
      this.drawcontext.clearRect(
        ...
        0,
        this.sub_div.offsetWidth,
        this.sub_div.offsetHeight
      );
      this.drawcontextSelect.clearRect(
        0,
        0,
        this.sub_div.offsetWidth,
        this.sub_div.offsetHeight
      ...
      for (let i = 0; i < this.drawList.length; i++) {
        ...
        let colorId = {
          r: this.drawList[i].SID % 256,
          g: parseInt(this.drawList[i].SID / 256) % 256,
          b: parseInt(this.drawList[i].SID / 256 / 256) % 25,
        };
        ...
        if (this.drawList[i].pointGroupData) {
          ...
          for (let k = 0; k < this.drawList[i].pointGroupData.length; k++) {
            if (this.drawList[i].pointGroupData[k].display == true) {
              this.drawList[i].type =
                this.drawList[i].pointGroupData[k].itemTypeToDisplay;
              drawItemTagType_ = this.drawList[i].pointGroupData[k].sign;
              if (
                this.drawList[i].pointGroupData[k].itemType == 5 &&
                this.drawList[i].pointGroupData[k].data
              ) {
                data = (
                  parseInt(this.drawList[i].pointGroupData[k].data) / 1000
                ).toString();
              } else {
        ...
        switch (this.drawList[i].type) {
          case "move":
            if (this.drawing) {
              if (
                this.drawMaxCount == 2 &&
                this.drawList[this.drawList.length - 1].position.length > 0 &&
                this.drawCount != this.drawMaxCount &&
                this.drawtype == this.drawList[i].type
              ) {
                this.drawLine(
                  this.drawcontext,
                  this.drawList[this.drawList.length - 1].position[
                    this.drawList[this.drawList.length - 1].position.length - 1
                  ],
                  this.lastMovePostion
                );
                this.drawMove(
            ...
            for (let j = 0; j < this.drawList[i].position.length; j++) {
              ...
              if (this.drawList[i].pointGroupData) {
                for (
                  var i_ = 0;
                  i_ < this.drawList[i].pointGroupData.length;
                  i_++
                ) {
              ...
              this.drawMove(
                ...
                7.5,
                color,
                this.drawList[i].Selected,
                data,
                name,
                drawItemTagType
              );
              this.drawMove(
                this.drawcontextSelect,
                this.drawList[i].position[j],
                this.pointScale,
                7.5,
                colorId
            ...
            if (this.drawing) {
              if (
                ...
                this.drawLine(
                  this.drawcontext,
                  this.drawList[this.drawList.length - 1].position[
                    this.drawList[i].position.length - 1
                  ],
                  this.lastMovePostion
                );
                this.drawLevel(
          ...
          case "text":
            ...
            if (this.drawList[i].position.length < 2) {
              this.drawText(
                this.drawcontext,
                this.drawList[i].position[0],
                this.drawList[i].TempPostion,
                this.drawList[i].text,
                1.0,
                color,
                false,
                this.drawList[i].Selected
          ...
          case "line":
            ...
            if (this.drawList[i].position.length == 2) {
              this.drawLine(
                this.drawcontext,
                this.drawList[i].position[this.drawList[i].position.length - 1],
                this.drawList[i].position[this.drawList[i].position.length - 2],
                2,
                1,
                this.baseColor,
                this.drawList[i].Selected
          ...
          case "cloud":
            ...
            var last = {
              x: last_temp.x * this.ResolutionScale * this.scale,
              y: last_temp.y * this.ResolutionScale * this.scale,
            };

            for (var k = 0; k < this.drawList[i].position.length; k++) {
              var Position_temp = this.rotate_XY_display(
                this.drawList[i].position[k]
              );

              points.push({
                x: Position_temp.x * this.ResolutionScale * this.scale,
                y: Position_temp.y * this.ResolutionScale * this.scale,
              });
            }
            ...
            if (this.drawList[i].position.length > 2) {
              ...
              if (
                this.drawList[i].position[0].x ==
                  this.drawList[i].position[
                    this.drawList[i].position.length - 1
                  ].x &&
                this.drawList[i].position[0].y ==
                  this.drawList[i].position[
                    this.drawList[i].position.length - 1
                  ].y
              ) {
                ...
                var position_ = this.rotate_XY_display(
                  this.drawList[i].position[0]
                );
      ...
      if (this.lastPostion) {
        if (this.drawList.length > 0) {
          if (this.drawList[this.drawList.length - 1].position[1]) {
            this.setNumber_inputPostion(
              this.drawList[this.drawList.length - 1].position[1]
            );
          } else {
    ...
    drawCloud(
      ...
      for (let i = 0; i < points.length - 1; i++) {
        ...
        for (let j = 1; j < Math.ceil(l / k); j++) {
          ...
          let item = {
            x: points[i].x + j * k * dir.x,
            y: points[i].y + j * k * dir.y,
          };
          drawcontext.arc(
            item.x,
            item.y,
      ...
      if (points.length > 0 && !finish) {
        ...
        let dir = {
          x: last.x - points[points.length - 1].x,
          y: last.y - points[points.length - 1].y,
        };
        dir = this.normalize(dir);
        ...
        for (let j = 1; j < Math.ceil(l / k); j++) {
          ...
          let item = {
            x: points[points.length - 1].x + j * k * dir.x,
            y: points[points.length - 1].y + j * k * dir.y,
          };
          drawcontext.arc(
            item.x,
            item.y,
    ...
    drawSelectImgMark(
      ...
      if (select) {
        ...
        drawcontext.drawImage(
          fz_img,
          position.x * this.ResolutionScale * this.scale - 9,
          position.y * this.ResolutionScale * this.scale - 9
        );
      }
    ...
    drawLine(drawcontext, Start_, End_, radius, scale, color, isSelected) {
      ...
      drawcontext.lineTo(
        End.x * this.ResolutionScale * this.scale,
        End.y * this.ResolutionScale * this.scale
      );
          ...
          drawcontext.fillText(
            ...
            position.x * this.ResolutionScale * this.scale +
              radius *
                ...
                this.Koeffzient,
            position.y * this.ResolutionScale * this.scale +
              10 * this.scale * this.ResolutionScale * this.Koeffzient
          );
        }
          ...
          drawcontext.fillText(
            ...
            position.x * this.ResolutionScale * this.scale +
              radius *
                ...
                this.Koeffzient,
            position.y * this.ResolutionScale * this.scale -
              4 * this.scale * this.ResolutionScale * this.Koeffzient
          );
        }
      }
    },
    ...
    drawText(
      ...
      let start = {
        x: position_start.x * this.ResolutionScale * this.scale,
        y: position_start.y * this.ResolutionScale * this.scale,
      };
      let end = {
        x: position_end.x * this.ResolutionScale * this.scale,
        y: position_end.y * this.ResolutionScale * this.scale,
      };
    ...
    create_Multiple_points() {
      ...
      if (this.drawList.length > 0) {
        if (this.drawList[this.drawList.length - 1].type == "text") {
          this.drawList[this.drawList.length - 1].text =
            this.$refs.number_input_input.value;
          this.drawFinish();
        } else {
          let count = 6;
          ...
          let dir = new THREE.Vector2(
            this.lastPostion.x -
              this.drawList[this.drawList.length - 1].position[0].x,
            this.lastPostion.y -
              this.drawList[this.drawList.length - 1].position[0].y
          );
          let length = dir.length();
          dir.normalize();
          count--;
          ...
          let lastPointPosition = {
            x: this.drawList[this.drawList.length - 1].position[0].x,
            y: this.drawList[this.drawList.length - 1].position[0].y,
          };
          ...
          for (let j = 1; j < count + 1; j++) {
            ...
            this.drawList.push(
              this.create(temp_type, this.drawItemId, newPosition)
            );
            this.drawID++;
          }
      ...
      for (let i = 0; i < this.drawList.length; i++) {
        if (this.drawList[i].pointGroupData) {
          for (let j = 0; j < this.drawList[i].pointGroupData.length; j++)
            if (
              this.drawList[i].pointGroupData[j].itemType == drawtype &&
              this.drawList[i].pointGroupData[j].sign == sign
            ) {
    ...
    enableTypeOne(drawtype, drawItemId, status) {
      ...
      for (let i = 0; i < this.drawList.length; i++) {
        if (
          this.drawList[i].typeNum == drawtype &&
          this.drawList[i].ItemId == drawItemId
        ) {
          this.drawList[i].display = status;
        } else {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (list[i].pointGroupData) {
          for (let j = 0; j < list[i].pointGroupData.length; j++) {
            list[i].pointGroupData[j].itemTypeToDisplay = this.getType(
              list[i].pointGroupData[j].itemType
            );
    ...
    rotate_XY(x, y) {
      ...
      if (this.type == "PDF") {
        Temp_Size = {
          width: 1024,
          height: 1024 * (this.oldImageSize.height / this.oldImageSize.width),
        };
      }
          ...
          if (this.oldImageSize.width != 0 && this.oldImageSize.height != 0) {
            for (let i = 0; i < this.drawList.length; i++) {
              if (this.drawList[i].display) {
                count++;
                for (let j = 0; j < this.drawList[i].position.length; j++) {
                  var position = this.rotate_XY_display(
                    this.drawList[i].position[j]
                  );
            ...
            var center_diff = {
              x: center_out.x - center.x,
              y: center_out.y - center.y,
            };
              ...
              case "PDF":
...

                {
                  ...
                  if (this.angle == 0 || this.angle == 180) {
                    ...
                    this.drawCanvasSelect.height = this.imageSize.height;
                    this.drawCanvasSelect.width = this.imageSize.width;
                    this.sub_div.style.left =
                      (width - this.sub_div.style) / 2 + "px";
                    this.sub_div.style.top =
                      (height - this.imageSize.height) / 2 + "px";
                  } else {
                    this.sub_div.style.height = this.imageSize.width + "px";
                    this.sub_div.style.width = this.imageSize.height + "px";
                    this.drawCanvas.height = this.imageSize.width;
                    ...
                    this.drawCanvasSelect.width = this.imageSize.height;
                    this.sub_div.style.left =
                      (width - this.imageSize.height) / 2 + "px";
                    this.sub_div.style.top =
                      (height - this.sub_div.style) / 2 + "px";
                  }
                }
