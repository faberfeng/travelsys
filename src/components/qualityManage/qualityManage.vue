    ...
    <div id="qualityChecking">
            <div class="topHeader">
                <div id="item-box-file">
                    ...
                    <router-link :to="'/liveConnect/qualityChecking'" class="label-item label-item-active">  
                    质量检查
                    </router-link>
                ...
                <sendMes :showBox="'true'" v-if="goingToSend" :dcid="''" :iscomment="true"  :selectugid="groupId" :holderid="value_monomer" v-on:showNameHide="showHide" :checkTypeName='checkTypeValue' :dirId='checkTypeId'   v-on:hide="hideSendMes" v-on:refresh='getCommunicationList' ></sendMes>
                                ...
                                <div class="projectListInfo">
                                    ...
                                    <div class="projectListText">
                                        ...
                                        <div class="comments" v-if="item.showResponse">
                                            <sendMes :showBox="(item.dcStatus==8||item.dcStatus==9)?true:false" :dcid='item.dcId' :keycomment="index" :iscomment="false" :selectugid="groupId" 
            ...
            <div id="edit">
                ...
                <el-dialog width="450px" title="检查类型选择" :visible.sync="checkTypeSelectDialog" @close="checkTypeSelectCancle">
                        <div class="tree">
                            ...
                            <el-select v-model="checkTypeId" @change="changeClassify" placeholder="请选择">
                                <el-option
...
export default {
    ...
    methods:{
        loadingTitle(){
          var vn=this;
          vn.routerList=vn.getSecondGradeList(vn.moduleList,'017','01701','/liveConnect/qualityChecking');
    ...
    collect(val,collect,index){
            ...
            axios({
            method:'POST',
            url:vm.BDMSUrl+'design/'+val+'/3/collect',
            headers:{
                'token':vm.token
            },
            params:{
      ...
      getComment(val,index,showResponse,reviewCount,dcStatus,reload,event){
        ...
        getDirectoryWithAll(){
            ...
            axios({
                url:this.BDMSUrl+'doc/getDirectoryWithAll',
                headers:{
                    'token':vm.token
                },
                params:{
                ...
                if(response.data.cd==0){
                    if(response.data.rt){
                        ...
                        vm.FileTree.forEach((item)=>{
                            if(item.dirName.indexOf('质量检查')>-1){
                                this.CheckTypeSubDirList=item.children;
                            }
                        })
                    }
                }
                console.log(this.CheckTypeSubDirList,'this.CheckTypeSubDirList');
                console.log(vm.FileTree,'数据0000');
            })
        },
        //寻找树形结构
        treeDiGui(){
      ...
      dcStatusConfirm(){
        ...
        axios({
            method:'POST',
            url:vm.BDMSUrl+'design/'+vm.dcStatus.obj.dcId+'/3/updateStatus',
            headers:{
                'token':vm.token
            },
        params:{
        ...
        sendChange(){
            if(this.goingToSend==false){
                ...
                var vm = this;
                vm.goingToSend =true;
                this.getDirectoryWithAll()
            }
        },
        changeClassify(){
            this.getClassificationsList.forEach((item)=>{
                if(item.id==this.checkTypeId){
                    this.checkTypeValue=item.name;
                }
            })
        },
        nodeClick(obj){
        ...
        //     })
        //   }
            this.checkTypeId=obj.dirId;
            this.checkTypeValue=obj.dirName;
            // var str;
      ...
      checkStatus(val){
        ...
        var vm = this
          vm.$confirm('您要删除当前所选主题？', '提示', {
                ...
                type: 'warning'
                  ...
                  axios({
                        method:'POST',
                        url:vm.BDMSUrl+'design/'+dcId+'/3/delete',
                        headers:{
                            'token':vm.token
                        },
                    }).then((response)=>{
        ...
        updateStatus(dcId,dcStatus,operate,index){
          ...
          if(dcStatus != 0){
                axios({
                    method:'POST',
                    url:vm.BDMSUrl+'design/'+dcId+'/3/updateStatus',
                    headers:{
                        'token':vm.token
                    },
                params:{
