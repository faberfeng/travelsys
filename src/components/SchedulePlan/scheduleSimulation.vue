  ...
  export default {
    ...
    methods: {
      ...
      updateProjectTaskTime(taskStart,taskEnd,id){
        axios({
          url:this.BDMSUrl+'schedule/'+this.projId+'/task/updateProjectTaskTime',
          method:'get',
          headers:{
            'token':this.token
          },
        ...
        var windowData=document.getElementById('ganttRight');
            // if(item.currentWidth>20){
              ...
              leftItem.addEventListener('mousedown',(e)=>{
                e.stopPropagation()
                this.leftStartX=e.screenX;
                this.leftWidth=item.offsetWidth;
                this.leftLeft=item.offsetLeft;
                leftItem.show=true;
              })
              windowData.addEventListener('mousemove',(e)=>{
                if(leftItem.show){
                    e.stopPropagation()
              ...
              windowData.addEventListener('mousemove',(e)=>{
                if(rightItem.show){
                    ...
                    item.style.width=this.startWith+rightEndX+'px';
                }
              })
              windowData.addEventListener('mouseup',(e)=>{
                if(rightItem.show){
      ...
      dragGantt(item){
         {
           var windowData1=document.getElementById('ganttRight');
           item.style.cursor='move';
            ...
            windowData1.addEventListener('mousemove',(e)=>{
                  if(item.upShow){
                    e.stopPropagation()
      ...
      clickGantt(item){
         item.addEventListener('click',(e)=>{
              const data=document.getElementById('ganttRight').childNodes;
              // item.upShow=false;
              this.itemClickShow=true;
              this.ganttItemId=item.rootData.taskId;
              for(let a=1,i=1;i<=data.length;i=3*a-1,a++){
                if(i>1){
                  data[i].style.border='';
                }
              }
